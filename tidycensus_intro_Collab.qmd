...
author: "Sano Tanna And Vlad Tomutiu" 
date: "`r Sys.Date()`"
format: html
editor: visual
toc: true
...
# Change names and use only GEOID for UH and Cham
oahu_universities <- oahu_poverty %>%
  filter(
    GEOID == "15003002701" | GEOID == "15003002801"
  ) %>%
  mutate(
    NAME = case_when(
      GEOID == "15003002701" ~ "UH Mānoa",
      GEOID == "15003002801" ~ "Chaminade University",
      TRUE ~ NAME
    )
  )

# zoom into just two schools with interactive map
oahu_uni_map <- ggplot(oahu_universities) +
  geom_sf_interactive(
    aes(
      fill = percent_povertyE,
      tooltip = paste(NAME, ": ", percent_povertyE, "%"),
...
girafe(ggobj = oahu_uni_map, width_svg = 7, height_svg = 4)
```
...
```{r, message=FALSE, warning=FALSE, results='hide'}
years <- 2014:2018  # specify years
names(years) <- years

oahu_education_2014_2023 <- map_dfr(years, ~{
  get_acs(
    geography = "county",
    variables = c(percent_bachelors = "DP02_0068P"),  # % 25+ with Bachelor's or higher
    state = "HI",
    county = "Honolulu",
    year = .x,
    survey = "acs5")
...
ggplot(oahu_education_2014_2023, aes(x = year, y = estimate, group = 1)) + 
  geom_line(color = "red", size = 1) + 
  geom_point(color = "red", size = 2) +
  labs(
    title = "Honolulu County Educational Attainment (ACS Rolling 4-year Estimates)",
    x = "ACS Release Year (4-year window)",
...
\*Data Missing leading to severe drop in educational attainment after 2019.

### Scatter Plot

Exploring the relationship between Educational Attainment and Gross Rent with a scatter plot

```{r, message=FALSE, warning=FALSE, results='hide'}
oahu_family_rent_and_education <- get_acs(
  geography = "tract",
  variables = c(median_rent = "DP04_0134E",
                percent_bachelorsE = "DP02_0068PE"),
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
...
ggplot(oahu_family_rent_and_education, aes(x = median_rent,y = percent_bachelorsE)) +
  geom_point() +
  geom_smooth() + # adds a regression line (LOESS)
  stat_cor() + # prints R and p
  theme_minimal() + 
  labs(title = "Comparing Rates of Higher Education to rent, Honolulu County",
       subtitle = "ACS 5-year, 2023 release",
       x = "median rent in Honolulu County",
       y = "Percent with a BA or higher",
       caption = "Shaded area represents margin of error around the estimate")
```
...
Looking at the scatter plot, we can see there is a small correlation between the educational attainment within specific Honolulu Countys and gross rent in those counties. I think this is accurate and makes a lot pof rent because for the most part the higher education you attain, the better your job opportunities will be. This is not always the case, but in general it is something that should correlate, so I think the scatter plot makes sense. The more education you have, the more opportunities open to get higher paying jobs and potentially allowing you to afford houses with higher rent.
